...
//! the `mft-wasm` crate (the browser build, which serves the same `Worker` over
//! a SharedArrayBuffer byte channel instead).
//!
...
/// The catalog name DuckDB sees in `ATTACH 'mft' (TYPE vgi, …)`. Defaults to
/// `mft`, but honors an explicit override so a test harness can rename it. Also
/// exports the variable so downstream SDK code observes the same default.
...
/// natively, the SAB byte channel in the browser.
