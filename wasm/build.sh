#!/usr/bin/env bash
# Build the browser (`worker:`) build of the mft VGI worker: mft-wasm compiled to
# wasm32-unknown-emscripten, linked by emcc into a MODULARIZE'd Web Worker module.
#
...
#   - a nightly toolchain with the rust-src component (rustup component add
#     rust-src --toolchain nightly) and the wasm32-unknown-emscripten target:
#     -Z build-std is required because the pthread build recompiles
#     compiler_builtins with atomics.
set -euo pipefail
cd "$(dirname "$0")/.."
...
LIB="target/$TARGET/release/libmft_wasm.a"
...
# thread-caching and scales about as well as native. Override with
# MALLOC=dlmalloc to compare.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which this worker overflows — the Arrow encoders nest deeply, and an overflow
# surfaces as a bare "memory access out of bounds" with no console output.
# INITIAL_MEMORY must cover main stack + every pthread stack up front, or startup
# stalls trying to grow shared memory while the pool is spawning.
echo "==> emcc link"
...
# Stage the canonical boot alongside the module (referenced, not vendored — it is
# transport ABI shared with every VGI worker; see VGI_WORKER_BOOT).
