...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-mft VGI
# worker, using a prebuilt standalone `haybarn-unittest` and the signed community
# `vgi` extension — no C++ build from source. See ci/README.md.
#
# Parameterized by TRANSPORT (default: subprocess), exercising the SAME suite over
# each transport the vgi extension supports — the only thing that changes is what
# LOCATION (VGI_MFT_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_MFT_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `mft-worker --http` (auto port; advertises `PORT:<n>` on
#               stdout), VGI_MFT_WORKER = http://127.0.0.1:<port>.
#   unix        start `mft-worker --unix <sock>` (advertises `UNIX:<sock>` on
#               stdout), VGI_MFT_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled mft-worker binary (used to launch the
#                     http/unix servers, and the stdio LOCATION).
# Optional:
...
# The committed $MFT fixtures live at the repo's data/ dir; the .test files
# reference them via ${VGI_MFT_DATA} so they resolve regardless of the runner's
# cwd (run-integration.sh cd's into a staging dir before invoking the runner).
export VGI_MFT_DATA="$REPO/data"

WORKER_BIN="${WORKER_BIN:-$REPO/target/release/mft-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_MFT_WORKER.
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the mft-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_MFT_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_MFT_SOCK:-/tmp/mft.$$.sock}"
      ...
      export VGI_MFT_WORKER="unix://$SOCK_PATH"
...
case "$TRANSPORT" in
  subprocess)
    export VGI_MFT_WORKER="${VGI_MFT_WORKER:-$WORKER_BIN}"
    ;;
  http)
    if [[ "${VGI_MFT_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_MFT_WORKER"
    else
...
: "${VGI_MFT_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_MFT_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_MFT_WORKER." >&2
  exit 1
fi
