...
#
# Dispatch the single vgi-mft image into one of its transports:
...
#
# The worker is stateless (it reads only the $MFT bytes it is handed and carries
# its resume cursor in the query state), so there is no /data to create and no
...
case "${1:-http}" in
  http)
    ...
    exec mft-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec mft-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec mft-worker "$@"
